<script lang="ts">
	...
	// Debug logging
	console.log('🎮 Player page data:', data);
	console.log('📊 My leagues:', data.leagues);
	console.log('🔍 Available leagues:', data.availableLeagues);
			...
			<Card.Content class="p-6">
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-3xl font-bold text-black mt-1">
							{data.leagues?.filter((l) => (l as any).league_owner === data.user?.id).length || 0}
						</p>
					</div>
					<div class="p-3 bg-[#2F91F6] rounded-lg">
					...
					<Card.Root class="border-2 border-white bg-white shadow-xl hover:shadow-2xl transition-shadow">
						<Card.Header>
							<div class="flex items-start justify-between">
								<div class="flex-1">
									...
									<Card.Description>
										{leagueData.participantCount || 0} participants
									</Card.Description>
								</div>
								{#if leagueData.league_owner === data.user?.id}
						...
						<Card.Content>
							<div class="flex items-center gap-2 text-sm text-gray-600">
								...
								<span>Created {new Date(leagueData.created).toLocaleDateString()}</span>
							</div>
						</Card.Content>
						<Card.Footer>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				{#each data.availableLeagues as league}
					...
					<Card.Root class="border-2 border-white bg-white shadow-xl hover:shadow-2xl transition-shadow">
						...
						<Card.Footer>
							<Button href="/fantasyleagues/{leagueData.id}" class="w-full bg-[#2F91F6] hover:bg-blue-600 text-white">
		...
		{:else}
			<Card.Root class="border-2 border-white bg-white">
				<Card.Content class="py-12 text-center">
					...
					<p class="text-gray-600 mb-6">
						There are no fantasy leagues available to join at the moment.
					</p>
				</Card.Content>
			</Card.Root>
		{/if}
